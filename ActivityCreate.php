...
if (isset ($_POST['activity']) && isset($_POST['record'])){
	...
	$record = (empty($_POST['record']) ? '' : (trim($_POST['record'])));
	$create = $model->createActivity($activity,$record,$username);
	echo "<p class='success'>Entry created for activity ${activity}! Return to the home page to see your new entry.</p>";
}
